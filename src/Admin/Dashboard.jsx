...
const Dashboard = () => {
  ...
  useEffect(() => {
    // 🔹 Check if 'bookings' or 'orders' is the correct key
    const bookings = JSON.parse(localStorage.getItem("bookings")) || [];  // Try 'bookings'
    const orders = JSON.parse(localStorage.getItem("orders")) || [];      // Try 'orders'
    const products = JSON.parse(localStorage.getItem("products")) || [];

    const allBookings = bookings.length > 0 ? bookings : orders; // Use 'bookings' if available, fallback to 'orders'

    setTotalBookings(allBookings.length);
    setPendingBookings(allBookings.filter(order => order.status === "Pending").length);
